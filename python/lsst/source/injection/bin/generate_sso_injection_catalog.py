...
This script generates a synthetic source injection catalog of sso orbital elements
from user supplied input parameters. The catalog may be printed to screen, ingested 
into a butler repository, or written to disk using the astropy Table API.

The orbital distribution is generated between bounds that are specified
by the user using the paramaters arguments: "a_limits", "e_limits", "i_limits",
"Omega_limits", "omega_limits", "M_limits". Each limit is a tuple of two values
specifying the lower and upper bounds of the distribution. Parameters that are
not set default to bounds set in the `generate_sso_injection_catalog` function.
