...
def get_reference_frame(input_exposure) -> GCRS:
    """
    Provides an astropy.coordinates reference_frame of the observer
    """
    obstime = input_exposure.visitInfo.date.toAstropy()
    location = EarthLocation.from_geodetic(lon=input_exposure.visitInfo.getLongitude().asDegrees(),
                                           lat=input_exposure.visitInfo.getLatitude().asDegrees(),
                                           height=input_exposure.visitInfo.getHeight().asDegrees())
...
def propagate_injection_catalog(orbits: Table, input_exposure: Exposure) -> Table:
    reference_frame = get_reference_frame(input_exposure)
